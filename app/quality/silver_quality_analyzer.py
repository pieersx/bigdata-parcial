...
class SilverQualityAnalyzer:
    ...
    def analyze(
        ...
        accepted_ratio = total_rows / (total_rows + quarantine_count) if total_rows + quarantine_count else 0.0
        ...
        scores = {
            ...
            "validez": accepted_ratio,
            "consistencia": accepted_ratio,
            "integridad": accepted_ratio,
            "actualidad": 1.0 if total_rows else 0.0,
            "disponibilidad": 1.0 if total_rows else 0.0,
            "exactitud": accepted_ratio,
        }
        checks = []
        for dimension in DIMENSIONS:
            ...
            checked = max(total_rows + quarantine_count, 1)
            passed = min(round(score * checked), checked)
            check = DataQualityCheck(
                check_id=f"silver_{dimension}_{table_name}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}",
                check_name=f"silver_{dimension}_{table_name}",
